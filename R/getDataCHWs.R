...
getDataCHWList<-function(d2_session) {
  ...
  url <- paste0(d2_session$base_url, "api/users?fields=firstName,surname,phoneNumber,userCredentials[username,disabled],organisationUnits[path]&paging=false")
  #url <- paste0(d2_session$base_url, "api/userRoles/ydBOUToSmET?fields=users[firstName,surname,phoneNumber,userCredentials[username],organisationUnits[id,level]]&paging=false")
...
getStep3Orgunits <- function(d2_session) {
  ...
  url <- paste0(d2_session$base_url, "api/dataSets/Gso59nSpsrG?fields=organisationUnits[id,name,contactPerson,phoneNumber]")
