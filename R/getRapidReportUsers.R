...
getRapidReportUsers <- function(d2_session = dynGet("d2_default_session",
  ...
  users <- datimutils:::api_get("api/userGroups/D0WkbASa55p?fields=users[phoneNumber,organisationUnits[id,name],userCredentials[lastLogin,username,disabled]",d2_session = d2_session) %>%
    purrr::pluck("users") %>%
