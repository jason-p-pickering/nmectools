...
writeAttachmentFiles <- function(d) {
  ...
  for (i in seq_along(donors)) {
    ...
    attachment_name.new <- paste(working_dir,
        "/topup_report_",
        format(as.Date(d$report_date), "%Y%m%d"), "_", donors[i], ".xlsx", sep = "")
  ...
  topup_data_attachment <- paste0(working_dir,
      "/topup_data_", format(as.Date(report_date), "%Y%m%d"), ".xlsx")
  ...
  d$users_bad_phonenumbers <- d$topup_users %>%
    ...
    topup_data_attachment <- paste0(working_dir,
        "/users_bad_phonenumbers_", format(as.Date(report_date), "%Y%m%d"), ".xlsx")
